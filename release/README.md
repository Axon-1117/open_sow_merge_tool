...
## 0. 当前版本
- 版本号：`2026-09-16.update95`
- Build Tag：`new172-premerge-formula-cache-save-guard`

### 本次修复
- 预合并保留公式计算结果缓存，涵盖采用公式和新增整张 Sheet；无修改时保留原始文件内容。
- 保存合并结果前全量检查公式缓存；缺失时停止覆盖并提示具体单元格，保留待保存操作。
- 修复缺失 Sheet 的比较与展示，允许在其他 Sheet 比较失败或尚未完成时手动保存已执行操作。
- 修复 Excel COM 混合数值、布尔值和空白写入，以及宽表三方横向滚动和局部视图位置恢复。

### 此前修复
- 大表差异视图现在会随窗口可用高度动态扩展虚拟视口，减少底部空白并在调整窗口大小后保持可见行与导航位置稳定。
- 新增工作簿级精确 Sheet 状态图；选中 Sheet 优先计算，其余 Sheet 后台完成精确相同/已改动判定，计算未完成时不再发布临时比对结果。
- 复制、覆盖、区域采用、结构修改、撤销/重做和保存统一接入精确就绪门禁；暂不可操作时会说明当前状态、阶段、原因和重试条件，并保证不产生隐式修改。
- 大表比对改用不可变 Sheet 快照、完整文件一致性快路径和虚拟结果窗口；193 个真实文件、2-way/3-way 共 1592 个 Sheet 的最终 GUI 验收全部低于 15 秒，最慢约 9.14 秒。
- 20,000 行差异视图只发布固定可见窗口，滚轮、翻页、拖动、缩略图及差异块跳转不读取 worksheet；1,000 行区域操作使用单次 overlay 事务并支持快速撤销/重做。
- 加强跨工作簿行样式、批注、超链接、公式、行列元数据和结构回放；XLSX/XLSM、ZIP/native 保存、失败恢复及真实 Excel 重开门禁均已通过。
- 对比视图会在后台检测没有保存计算缓存的公式；检测到后弹窗询问是否调用本机 Excel 重算。选择重算时会显示不可关闭的模态进度窗，完成前禁止操作主界面；重算只作用于临时副本，不会写回原始工作簿。选择跳过或 Excel 不可用时，工具仍可安全打开文件，并显示公式文本。
...
## 6. 常见问题
### 6.1 为什么看到的是公式而不是最终值？
- Excel 文件会把公式和“上次计算结果”分别保存。工具优先显示后者；若文件没有保存该结果，就必须通过桌面版 Excel 重算。
- 检测到这类单元格后，工具会询问是否重算；选择“是”后会在不可操作主界面的进度窗内等待完成。原文件不会被修改。若仍显示公式，请确认桌面版 Excel 可正常启动且当前 Windows 登录会话可使用 Office，然后点击顶部的“重算并刷新”。

### 6.2 保存失败（Permission denied）
