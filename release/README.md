...
## 0. 当前版本
- 版本号：`2026-08-14.update76`
- Build Tag：`branch-recency-and-shell-order`

### 多分支 SVN 提交（新增）
- 启动 `sow_merge_tool.exe --branch-submit "<文件或文件夹>"`，或通过资源管理器右键进入“TortoiseSVN 风格多分支提交工作台”。
- 右键 `.xlsx` 会递归扫描其所在文件夹；右键文件夹或文件夹空白处会递归扫描该文件夹。变更表同时显示 Excel 和其他文件，但只有符合安全规则的 `.xlsx` 能进入批次。
- 目标分支从同一 SVN 仓库、同一工作副本布局中动态发现；支持搜索、全选、全不选、收藏和任意数量复选，包括 `master`。源分支和目标分支均按 SVN 最近变更时间倒序排列。
- 变更表支持已版本化、新增、删除、修改、文件、目录等快捷筛选；冲突、切换、属性修改、外部定义、重解析点及无法确认的状态都会 fail-closed 阻断。
- 提交说明可读取当前仓库对应的 TortoiseSVN 最近消息，并提供“显示日志”和“粘贴所选文件名”。
- “预检查”会先生成“目标分支 × 文件”动作矩阵。修改文件只投影“源分支 pristine → 源工作文件”的语义增量；新增和删除必须分别满足目标不存在、目标与删除前基线一致等条件。
- 正式执行是可恢复的分步事务：先打开源分支原生提交窗口，逐文件对账后再逐个打开目标分支窗口；目标说明追加 `[MultiBranchSync] batch=... source=...@r...`。
- 不以 TortoiseSVN 进程退出码判断提交事实。每个窗口关闭后都会重新核对 SVN 状态、pristine、文件哈希和版本；用户只勾选部分文件时立即停止并拆出已提交子批次。
- 每次写入目标工作副本前先保存事务日志、原文件和候选哈希。崩溃或取消后可继续批次，或仅在文件仍与候选完全一致时恢复未提交现场；已进入 SVN 仓库的版本不会自动回滚。
- 自动合并范围仍为 `.xlsx` 的值、公式和行新增/删除；重命名以及 `.xlsm`、工作表/列/合并单元格/样式等结构变化会阻断并提示使用人工合并或 TortoiseSVN Repair Move。

### 资源管理器右键入口（新增）
- 双击 `install_context_menu.bat` 后，会分别为 `.xlsx` 文件、文件夹和文件夹空白处注册“多分支 SVN 提交”。Windows 11 上该入口可能位于“显示更多选项”中。
- 工具会自动识别 SVN 工作副本、源分支、仓库 URL 和扫描范围，并在后台递归列出该范围内的全部更改，支持一次选择多个 Excel 文件。
- 双击 `uninstall_context_menu.bat` 只删除本工具自己的三个注册表键；完整的 `install.bat` / `uninstall.bat` 会同时安装或移除 TortoiseSVN Diff/Merge 配置与右键入口。
- 右键菜单写入 `HKCU` 当前用户注册表，不需要管理员权限。
- 右键菜单不再使用 `Position=Top` 强制置顶，按 Windows Shell 的自然 verb 顺序进入 SVN 操作区并排列在 SVN 扩展项之前；重新安装会清理旧版本遗留的置顶值。

### 本次修复
- 修复部分 SVN 导出的有效 XLSX 缺少 worksheet `<dimension>` 元数据时被误判为 `1×1` 表的问题；现在会按实际 XML 单元格计算行列范围，避免把仍存在的后续列批量显示为“已删除”。
- 修复 2-way 查看一次提交对比时 Mine 与旧版本侧缺少 author 的问题；现在会在文件标准化前保留 SVN 原始身份，显示 author/revision，并在悬停详情中保留完整路径和不可用原因。
- 2-way 全局替换现在只会阻止“歧义列中确实存在真实差异”的情况；空白分隔列或重复列在左右完全相同时会被安全跳过。
- 歧义列含真实差异时会显示 Sheet、逻辑列、歧义原因、左右物理列、差异数量和示例位置，并提供返回检查或明确的左右整 Sheet 覆盖选项；整 Sheet 覆盖前会再次提示目标 Sheet 的结构和独有内容将被替换。
- 整 Sheet 覆盖复用 Sheet 操作记录，支持 A→B/B→A 两个方向、单步撤销和结构刷新；保存回放保留原 Sheet 位置，并修复中文 JSON 操作在 Windows PowerShell 下的 UTF-8 读取。
- 保存前后继续执行 OOXML/ZIP 完整性校验和可重开校验；中文文本按原值保存，避免 Excel 打开时出现修复提示或乱码。
- C 区行对比改为使用下方完整可用宽度，不再被限制为单个主视区宽度；C 区横向滚动独立于主视区，并保持表头、正文和逻辑列对齐。
- 用户点击“只看差异”且需要生成精确快照时，会显示带 Sheet 名称、处理阶段、行数、百分比、耗时和“取消”按钮的模态进度窗；计算期间当前 Sheet 保持只读并禁止切换 Sheet，取消后恢复原全量视图，迟到后台结果不会覆盖界面。
- “只看差异”复选框保持固定文案和固定位置；差异块序号、总数及待处理数量移动到独立导航栏，不再因勾选状态或计算文案导致工具栏大幅位移。
- WorldMonster 真实三方冷路径取消/重试连续验收：成功路径点击回调约 31–44ms，进度窗约 64–77ms 出现，Tk heartbeat 最大间隔约 132–145ms；在约 27% 进度取消时，取消响应约 27–32ms、worker 约 21–22ms 退出，原全量视图与 Sheet 切换立即恢复，迟到结果被拒绝。重试进度完整覆盖 0–100%，复选框位置无漂移。
