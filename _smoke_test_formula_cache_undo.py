"""Three-way formula-cache adopt/undo/output regression on declared records."""
...
def _owned_case():
    ...
    temporary = tempfile.TemporaryDirectory(prefix="sow_formula_cache_undo_")
    ...
    try:
        ...
        if cleanup_errors:
            message = "formula-cache undo cleanup failed: " + "; ".join(cleanup_errors)
...
def _wait_operation_ready(app, sheet: str, deadline: float):
    ...
    while time.monotonic() < deadline:
        ...
        if (
            ...
            return view
        time.sleep(0.01)
    raise AssertionError(("S1 did not become operation-ready", app._sheet_exact_entry(sheet), view))
...
def main():
    ...
    with _owned_case() as owned:
        ...
        merged = os.path.join(owned.root_dir, "merged.xlsx")
        _make_formula_book(base, 3)
        _make_formula_book(mine, 1)
        _make_formula_book(theirs, 2)
        owned.input_hashes = {path: _sha256(path) for path in (base, mine, theirs)}
        ...
        assert logical_col == 2
        assert view.row_pairs[pair_idx][0] == row_a
        ...
        assert app.ws_a_val("S1").cell(row_a, 2).value == 2
        assert view._copy_selected_row("BASE2A", override_pair_idx=pair_idx, override_cols={logical_col})
        assert app.ws_a_val("S1").cell(row_a, 2).value == 3
    ...
    print("SMOKE_FORMULA_CACHE_UNDO_OK", flush=True)
