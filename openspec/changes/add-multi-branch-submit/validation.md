...
## Execution-layer evidence (2026-08-13)
...
- Real read-only WC discovery at `C:\sow_main\excel` found 22 same-repository branches, disabled `master`, confirmed repository UUID `509b88cb-e3bb-49fc-85e7-49e888d66b00`, and read 24 recent TortoiseSVN messages.
...
- `build_exe.ps1 -SkipPublish` generated the `2026-08-13.update75` single EXE and ZIP. The installed EXE SHA256 is `5E1445EE3D7D8D7A7B4C4CCD8767144ECDC9226CDDA2CE3DF70EE0A26EB2DFFF`; all 12 deployed files match the release directory.
- Context-menu round trip installed exactly three owned keys and then removed exactly those keys. An adjacent sentinel key survived. Final commands point to `C:\sow_main\excel\excel_merge_tool\sow_merge_tool.exe` with `%1`, `%1`, and `%V` respectively.
